...
#define UCA1_BR0  156  // Value of UCA1BR0 register                    24MHz / 16 / 9600 (see User's Guide)
#define UCA1_BR1  0    // Value of UCA1BR1 register
#define UCA1_BRS  0    // Value of UCBRS field in UCA1MCTL register    Modulation UCBRSx=0, UCBRFx=4
...
void UartInit(void)
{
        P1SEL |= BIT4+BIT5;                          // P4.4,5 = USCI_A1 TXD/RXD
        UCA0CTL1 |= UCSSEL_2;                        // SMCLK
        UCA0BR0 = UCA1_BR0;                          // 24MHz / 16 / 9600 (see User's Guide)
        UCA0BR1 = UCA1_BR1;
        UCA0MCTL |= UCA1_OS | UCA1_BRF | UCA1_BRS;   // Modulation UCBRSx=0, UCBRFx=4
...
uint16_t UartReceive(uint8_t* buf)
{
    uint16_t i, count;
   ...
   // UCA1IE &= ~UCRXIE;
    ...
    UartRcvBufIndex = 0; //move index and threshold back
